...
from src.config.settings import PROJECT_ID, DATASET_ID, BASE_COORS, BASE_WR_KILOMETRIERT, ERROR, CREDENTIALS_PATH
    ...
    def update_base_area_code_kilometrierung_table(self, base_coors_wr_kilometriert:str, url: str, batch_size: int, offset: int = 0):
        ...
        while True:
            ...
            try:
                ...
                job.result()
                logger.success(f"Data has been loaded successfully into {BASE_WR_KILOMETRIERT}")
            ...
            try:
                ...
                job.result()
                logger.success(f"Data has been loaded successfully into {ERROR}")
            except Exception as e:
                logger.error(f"Error writing to BigQuery table: {e}")
            ...
            logger.success(f"Dataframe from {offset} to {offset + batch_size} has been completed!")
            offset += batch_size
